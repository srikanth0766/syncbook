...
def create_booking(db: Session, booking_data: BookingCreate) -> BookingModel:
...
def update_booking(db: Session, booking_id: str, booking_data: BookingUpdate) -> Optional[BookingModel]:
    booking = get_booking_by_id(db, booking_id)
    if not booking:
        return None

    # Check dates if updating dates
...
def cancel_booking(db: Session, booking_id: str) -> Optional[BookingModel]:
    booking = get_booking_by_id(db, booking_id)
    if not booking:
        return None
